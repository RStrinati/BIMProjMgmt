...
import { ArrowDownward, ArrowUpward, PushPin, PushPinOutlined } from '@mui/icons-material';
import type { UserFieldView } from './UserFieldRegistry';
import { USER_FIELD_MAP } from './UserFieldRegistry';

type UserColumnsPopoverProps = {
  view: UserFieldView;
...
export function UserColumnsPopover({
  view,
  anchorEl,
  onClose,
  visibleFieldIds,
  ...
  togglePinnedField,
  resetToDefaults,
}: UserColumnsPopoverProps) {
  const open = Boolean(anchorEl);
  const orderedFields = orderedFieldIds.map((id) => USER_FIELD_MAP.get(id)).filter(Boolean);
      ...
      <Stack spacing={1} sx={{ p: 1 }}>
          ...
          {orderedFields.map((field) => {
            ...
            return (
              <ListItem
                ...
                secondaryAction={(
                  <Stack direction="row" spacing={0.5}>
                    {view === 'list' && (
                      <IconButton size="small" onClick={() => togglePinnedField(field.id)} aria-label={`pin-${field.id}`}>
              ...
              >
                ...
                <ListItemText primary={field.label} secondary={field.format} />
              </ListItem>
            );
          })}
        </List>
        ...
        <Box sx={{ px: 1 }}>
          <Typography variant="caption" color="text.secondary">
            Changes apply to the master users list.
          </Typography>
        </Box>
      </Stack>
    </Popover>
