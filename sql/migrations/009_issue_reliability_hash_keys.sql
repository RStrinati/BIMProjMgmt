...
BEGIN
    IF COL_LENGTH('dbo.Issues_Current', 'issue_key_hash') IS NULL
    BEGIN
        ALTER TABLE dbo.Issues_Current
            ADD issue_key_hash AS HASHBYTES(
                'SHA2_256',
    ...
    IF EXISTS (
        ...
        WHERE name IN ('pk_issues_current', 'pk_issues_current_source')
...
BEGIN
    IF COL_LENGTH('dbo.Issues_Snapshots', 'issue_key_hash') IS NULL
    BEGIN
    ...
    IF EXISTS (
        SELECT 1
        FROM sys.indexes
        WHERE name IN ('uq_issues_snapshots_key_date', 'uq_issues_snapshots_source_date')
