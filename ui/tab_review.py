...
def build_review_tab(tab, status_var):
    ...
    license_start = DateEntry(tab, width=12)
    license_start.pack(padx=10, pady=2, anchor="w")
    license_duration = ttk.Entry(tab, width=10)
    license_duration.pack(padx=10, pady=2, anchor="w")
    ...
    create_horizontal_button_group(
        ...
        [
            ...
            (
                "Launch Gantt Chart",
                lambda: launch_gantt_chart(
                    cmb_projects.get().split(" - ")[0], cmb_cycles.get()
                ),
            ),
        ],
    )
